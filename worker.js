...
async function handle(request, env) {
  ...
  if (path === "/api/member/claim" && request.method === "POST") {
    ...
    // 1端末につき、同時に利用できる有効コードは1つだけ
    const linked = await env.DB.prepare(
      "SELECT code FROM member_codes WHERE device_id=? AND status='active' AND expires_at>=? AND code<>? LIMIT 1"
    ).bind(deviceId, now, code).first();

    if (linked) {
      return json(
        { ok: false, error: "device_already_linked", linkedCode: linked.code },
        409
      );
    }
    ...
    const result = await env.DB.prepare(
      `UPDATE member_codes
       ...
       WHERE code=?
         AND (status='unused' OR expires_at < ? OR device_id=?)
         AND NOT EXISTS (
           SELECT 1 FROM member_codes
           WHERE device_id=?
             AND status='active'
             AND expires_at>=?
             AND code<>?
  ...
  if (path === "/api/places" && request.method === "GET") {
    ...
    const stmt = category
      ? env.DB.prepare("SELECT * FROM places WHERE active=1 AND category=? ORDER BY updated_at DESC").bind(category)
      : env.DB.prepare("SELECT * FROM places WHERE active=1 ORDER BY updated_at DESC");
  ...
  if (path === "/api/admin/places" && request.method === "GET") {
    ...
    const rows = (result.results || []).map((p) => ({ ...p, aliases: JSON.parse(p.aliases || "[]") }));
    return json({ ok: true, places: rows });
  }
  ...
  if (path === "/api/admin/places" && request.method === "POST") {
    ...
    const result = await env.DB.prepare(
      `INSERT INTO places(category,name,aliases,score,label,comment,point,reservation,maps_query,review_query)
       VALUES (?,?,?,?,?,?,?,?,?,?)`
    ).bind(
      category, name, JSON.stringify(aliases),
      Math.max(0, Math.min(100, Number(b.score || 80))),
      String(b.label || "📚ウシ評価（事前調査）"),
      String(b.comment || ""), String(b.point || ""), String(b.reservation || ""),
      String(b.mapsQuery || name), String(b.reviewQuery || `${name} reviews official`)
    ).run();
  ...
  if (path.match(/^\/api\/admin\/places\/\d+$/) && request.method === "PUT") {
    ...
    const b = await bodyJson(request);
    await env.DB.prepare(
      `UPDATE places SET category=?,name=?,aliases=?,score=?,label=?,comment=?,point=?,
       reservation=?,maps_query=?,review_query=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`
    ).bind(
      b.category, b.name, JSON.stringify(b.aliases || []), Number(b.score || 80),
      b.label || "", b.comment || "", b.point || "", b.reservation || "",
      b.mapsQuery || b.name, b.reviewQuery || `${b.name} reviews official`,
